...
20.times do |index|	#cree 20 gossip en reference avec user
	Gossip.create(title: "Gossip#{index}",content: Faker::TheRock.fact, user: User.all.sample)
end

10.times do 		#cree 10 tag
	Tag.create(title: "##{Faker::Games::FF14.name}")
end
...
20.times do	#creer 20 messages en reference avec l'utilsateur sender et creer un nombre aleatoire de receveur user
	message = PrivateMessage.create(sender: User.all.sample, content: Faker::Movies::Matrix.quote)
	rand(1..6).times do
